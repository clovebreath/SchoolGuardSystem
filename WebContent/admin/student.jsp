		...
		<div class="col-md-12 column">
			<nav class="navbar navbar-default" role="navigation">
				...
				<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					...
					<form class="navbar-form navbar-left" role="search" action="student.jsp"  method="GET">
       ...
       if(request.getParameter("id")!=null){
    	 ...
    	 //**连接数据库**
			try{
			    ...
			    String user = "root";
			    String pwd = "123456";
			    String sql="SELECT sid,sname,spic,class,wname,canleave,sgender FROM schoolsys.student left join schoolsys.worker on student.tid=worker.wid where sid=\"" + request.getParameter("id")+ "\"";
			 //   String sql = "SELECT id,name,identity,status,sname,wname,time FROM schoolsys.record left join worker on record.tid=worker.wid left join student on record.sid=student.sid ORDER BY id desc";
			    ...
			    if(rs.next()){
			        ...
			        <tr>
			 	        ...
			 	        <td><img style="height:100px;width: auto;" src=<%="data:image/jpeg;base64,"+rs.getString("spic") %>/></td>
			 	        <td><%=rs.getString("canleave") %></td>
			 	        <% if(rs.getString("canleave").equals("Y")) {%>    
			 	        <td><button class="btn btn-block btn-default"  value="N"  id=<%=rs.getString("sid") %> >
			 	        销假
			 	        </button></td>
			 	        <%}else{ %>
			 	        <td><button  class="btn btn-block btn-default"  value="Y"  id=<%=rs.getString("sid") %> > 
			 	        请假
			 	        </button></td>	        
			  			<%} %>
			 <script type="text/javascript">
			 var btnId=<%=rs.getString("sid")%>;
			...
			try{
			    ...
			    String user = "root";
			    String pwd = "123456";
			    String sql="SELECT sid,sname,spic,class,wname,canleave,sgender FROM schoolsys.student left join schoolsys.worker on student.tid=worker.wid";
			 //   String sql = "SELECT id,name,identity,status,sname,wname,time FROM schoolsys.record left join worker on record.tid=worker.wid left join student on record.sid=student.sid ORDER BY id desc";
			    Class.forName("com.mysql.jdbc.Driver");
			    Connection conn = DriverManager.getConnection(connectString, user, pwd);
